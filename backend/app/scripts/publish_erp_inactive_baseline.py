"""Publish the current ERP-inactive product baseline once.

This is intentionally a one-time migration. Products synchronized after the
cutoff continue to enter the normal Draft workflow.

Usage:
    python -m app.scripts.publish_erp_inactive_baseline
    python -m app.scripts.publish_erp_inactive_baseline --apply
    python -m app.scripts.publish_erp_inactive_baseline --rollback
"""
...
AUDIT_ACTION = "erp_inactive_product_baseline_published"
...
def apply() -> dict[str, int | str]:
    with SessionLocal() as db:
        ...
        if marker is not None:
            raise RuntimeError(
                f"The inactive ERP baseline was already published at {marker.created_at.isoformat()}."
            )
        ...
        result = db.execute(
            ...
            .where(
                CatalogueEntry.product_id.in_(_eligible_product_ids(published_at)),
                CatalogueEntry.workflow_status != "published",
            )
            .values(
        ...
        db.add(
            AuditLog(
                ...
                module="catalogue",
                status="success",
                identifier="inactive-gms-erp-products",
                details={
                    "published_count": changed,
                    "cutoff": published_at.isoformat(),
                    "scope": "inactive ERP products existing at the cutoff",
...
def rollback() -> dict[str, int | str]:
    with SessionLocal() as db:
        ...
        if marker is None or not marker.details:
            raise RuntimeError("No inactive ERP baseline publication marker was found.")
